        ...
        Form {
            Section {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Picker("Webhook Mode", selection: $settings.webhookType) {
                    ...
                    Picker("HTTP Method", selection: $settings.webhookMethod) {
                        Text("POST (Sends JSON payload)").tag("POST")
                        Text("GET (Performs ping request)").tag("GET")
                    }
                    .pickerStyle(.segmented)
                    .frame(width: 300)
                }
